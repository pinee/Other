...
Q(:,1,n+1)=(Q(:,1,n));
%2nd order Steger and Warming flux vector explicit scheme
...
Q(:,2,n+1)=Q(:,2,n)-0.5*(dt/dx)*(1/S(2))*(Fp(:,2,n)-Fp(:,1,n))+((1/S(j))*dt*(H(:,2,n)));
for j=3:JM-2
Q(:,j,n+1)=Q(:,j,n)-0.5*(dt/dx)*(1/S(j))*(Fp(:,j-2,n)-4*Fp(:,j-1,n)+3*Fp(:,j,n))+((1/S(j))*dt*(H(:,j,n)));
end
Q(:,JM-1,n+1)=Q(:,JM-1,n)-0.5*(dt/dx)*(1/S(JM-1))*(Fp(:,JM-1,n)-Fp(:,JM-2,n))+((1/S(JM-1))*dt*(H(:,JM-1,n)));
